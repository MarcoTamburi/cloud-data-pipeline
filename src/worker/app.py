...
@app.post("/lambda/invoke/2015-03-31/functions/<function_name>/invocations")
def lambda_runtime_invocations(function_name: str):
    """
    Adapter for Step Functions Local Lambda endpoint.
    It calls our internal steps based on <function_name>.
    The request body is the Lambda event (JSON).
    """
    payload = request.get_json(silent=True) or {}

    # Dispatch to the same logic used by our endpoints
    if function_name == "ingest":
        # reuse ingest_route logic without HTTP hop
        ensure_dirs()
        ...
        nyc_population = int(payload.get("nyc_population", 8_500_000))
